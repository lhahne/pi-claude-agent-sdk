...
- **Fix: a turn whose prompt pi re-rendered with a different tool list is recovered instead of failed** — pi renders `before_agent_start`'s `systemPrompt` from the tool loadout as it stands mid-dispatch, then corrects `selectedTools` to the live loadout before writing the transcript's sections. An extension that calls `setActiveTools()` from its own handler therefore leaves the prompt the bridge records differing from the transcript the provider is handed — by that tool's snippet and guidelines, and nothing else. The provider's prompt matched no capture, so the guard failed the turn; every print, RPC and sub-agent run was affected for anyone with such an extension. `@juicesharp/rpiv-ask-user-question` is the real-world case, since it strips its tool whenever `ctx.hasUI` is false. `resolveOrDerive` now falls back to the capture recorded during that same turn, but only when the two prompts are line-subsets of each other — the shape a changed tool list produces. A prompt that merely *should* be related still fails loudly, and a tie between concurrent agent runs is refused rather than guessed, because choosing by recency would hand one agent another's context files. `docker/live-check.sh` pins it against the real extension.
- **Fix: pi 0.86 compatibility — providers now receive a `TranscriptContext`** — pi 0.86 folds the system prompt and tool declarations into a leading `system` message before calling a custom provider, so `context.systemPrompt` and `context.tools` no longer exist. The bridge treated that message as conversation history, converted it to zero Anthropic records, and `cc-session-io` wrote no file for an empty session — then resumed the UUID anyway, so every turn died with `No conversation found with session ID`. It now normalizes once at the provider boundary (`src/pi-context.ts`), reading the prompt with `getCurrentSystemPrompt()` and the tools with `getCurrentTools()`, and strips system messages from history so the session cursor, the history/prompt split and the message conversion share one index space. Mid-conversation prompt and tool changes are replayed. Requires pi 0.86+; the peer range is now `>=0.86.0` rather than `>=0.82.1`, which is what let the break through silently.
- **Fix: a non-empty history that converts to nothing now fails at the cause** — `convertAndImportMessages` refuses to write an empty session rather than letting the caller resume a file that was never created, and `convert.ts` skips `system` messages explicitly instead of silently dropping them. Both are guards against the same class of silent failure recurring.
- **Tests: Docker matrix against real Claude Code** — `docker/run-matrix.sh` runs the released 0.8.6 bridge and this tree against pi 0.86.1 in throwaway containers, asserting the bridge's internal invariants (`tools>0`, non-empty conversion, session file written, no missing-session resume) as well as the answers. The 0.8.6 row is pinned as an expected failure so the fix is validated against a reproduced bug. `unit-pi-streamfn-inventory.mjs` is now keyed by pi minor version and fails on an unreviewed one, so a pi bump means reading its dist rather than hearing about it from a user.
